...
//    [DLTask getm3u8File:_urlArray[1]];
}
...
-(void)DownLoadButionAction:(UIButton *)sender
{
    ...
    if ([fm fileExistsAtPath:filePath]) {
        ...
        NSLog(@"%ld,%ld,%ld",self.totalFile,self.curFileIndex,self.offset);
    ...
    }else
    {
        ...
        // 解析
        NSArray * downLoadList = [DLTask parsem3u8File:filePath];
        NSLog(@"%ld",downLoadList.count);
        NSString * DocumentPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
        NSString * dataBasePath = [DocumentPath stringByAppendingPathComponent:@"DataBase.sqlite"];
        DataBaseTool * db = [DataBaseTool shareDataBase];
        [db connectDB:dataBasePath];
        
        [db execDMLSql:[NSString stringWithFormat:@"update DownLoadTaskTable set totalFile = %ld,curFileIndex = %d, offset = %d where url = '%@'",downLoadList.count,0,0,_urlArray[sender.tag -100]]];
